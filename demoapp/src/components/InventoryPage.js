...
const InventoryPage = ({ searchQuery }) => {
  ...
  // useEffect(() => {
  //   axios.get(fetchInventoryList());
  // }, []);

//   useEffect(() => { axios.get(`http://localhost:5296/api/Inventory`)
...
// }, []);
  ...
  //   try {
  //     setLoading(true);
  //     const response = await axios.get(`http://localhost:5296/api/Inventory`);
  //     if (response.ok){
  ...
  const fetchInventoryList = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://localhost:7274/api/Inventory`);
  ...
  const deleteInventory = async (inventoryId) => {
    try {
      ...
      await axios.delete(`https://localhost:7274/api/Inventory/${inventoryId}`);
      alert("Inventory deleted successfully!");
      setInventories((prevInventories) =>
        prevInventories.filter((item) => item.inventoryID !== inventoryId)
      );
    } catch (error) {
  ...
  const filteredInventories = inventories.filter((item) =>
    item.bookManagement?.title?.toLowerCase().includes((searchQuery || "").toLowerCase())
  );

  const handleInventoryAdded = () => {
    setShowAddInventoryModal(false);
    fetchInventoryList(); // Refresh the inventory list after adding a new inventory
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="action-button " onClick={() => setShowDeleteBookModal(true)}>
            Delete Book
        </button>
        <button
          className="action-button"
            ...
            <tbody>
              {filteredInventories.map((item) => (
                <tr key={item.inventoryID}>
                  ...
                  <td className="p-2 border text-center">{item.bookManagement?.title}</td>
                  <td className="p-2 border text-center">{item.quantity}</td>
                  <td className="p-2 border text-center">{item.notifyLimit}</td>
                  <td className="p-2 border text-center space-x-2">
                  {/* <div className="button-container"> */}
                    <button className="action-button update-stock"
                      onClick={() => handleUpdateStockClick(item.bookID)} // Pass the bookId to the function
                    >
                      Add Stock
                    </button>
                    <button className="action-button update-notify"
                      onClick={() => handleUpdateNotifyLimitClick(item.inventoryID)} // Pass the inventoryId to the function
                    >
                      Update Notify Limit
                    </button>
                    <button
