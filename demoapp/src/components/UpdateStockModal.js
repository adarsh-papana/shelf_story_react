...
const UpdateStockModal = ({ bookId, onClose, onStockUpdated }) => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get(`https://localhost:7274/api/BookManagement/${bookId}`); // Replace with your API endpoint
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(`https://localhost:7274/api/Inventory/add-stock?bookId=${bookId}&quantity=${quantity}`);
